...
class _FuturePageState extends State<FuturePage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(
      () => Provider.of<FutureController>(context, listen: false).init(),
    );
  }

  @override
  Widget build(BuildContext context) {
    final controller = Provider.of<FutureController>(context);
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            Row(
              children: [
                const Text('Laverage', style: TextStyle(color: Colors.white)),
                const SizedBox(width: 12),
                Container(
                  padding: const EdgeInsets.symmetric(
                    horizontal: 16,
              ...
              children: [
                Expanded(
                  child: GestureDetector(
                    onTap: () async {
                      await controller.placeOrder("long");
                    },
                    child: Container(
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () async {
                      await controller.placeOrder("short");
                    },
                    child: Container(
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(
                ...
                children:
                    controller.orders.map((order) {
                      return Padding(
                        ...
                        child: Text(
                          '${order.type.toUpperCase()} - ${order.entryPrice} - ${order.quantity} - ${order.timestamp.toLocal()}',
                          style: const TextStyle(
                            color: Colors.white54,
                            fontSize: 12,
                          ),
